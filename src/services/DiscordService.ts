...
export async function sendStatusMessage(): Promise<void> {
    ...
    const client: Discord.Client = new Discord.Client({ intents: ['GUILDS'] });
    client.login(process.env.DISCORD_TOKEN);

    client.on('ready', async () => {
        const botChannel = client.channels.cache.find((channel: any) => channel.name === process.env.DISCORD_CHANNEL_NAME) as Discord.TextChannel;
